#import "LibellumRootListController.h"

@implementation LibellumRootListController
	- (instancetype)init {
		self = [super init];
	...
	- (void)viewDidLoad {
		...
		LibellumHeaderView *header = [[LibellumHeaderView alloc] initWithTitle:@"Libellum" subtitles:subtitles bundle:[self bundle]];
	...
	- (void)viewDidAppear:(BOOL)animated {
		...
		if(!self.navigationItem.titleView) {
			LibellumAnimatedTitleView *titleView = [[LibellumAnimatedTitleView alloc] initWithTitle:@"Libellum" minimumScrollOffsetRequired:90];
	...
	- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
		if([self.navigationItem.titleView respondsToSelector:@selector(adjustLabelPositionToScrollOffset:)]) {
			[(LibellumAnimatedTitleView *)self.navigationItem.titleView adjustLabelPositionToScrollOffset:scrollView.contentOffset.y];
		}
	}
	...
	- (void)manageBackup:(PSSpecifier *)specifier {
		...
		LibellumNoteBackupViewController *backupViewController = [[LibellumNoteBackupViewController alloc] init];
