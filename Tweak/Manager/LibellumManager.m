...
@implementation LibellumManager {
    ...
    - (instancetype)init {
        if(self = [super init]) {
            _preferences = [HBPreferences preferencesForIdentifier:@"com.lacertosusrepo.libellumpreferences"];
    ...
    - (void)createPages {
        ...
        _notesByIndex = ([NSKeyedUnarchiver unarchivedObjectOfClasses:[NSSet setWithObjects:[NSMutableDictionary class], [NSAttributedString class], [NSNumber class], nil] fromData:[self.preferences objectForKey:@"notes"] error:nil]) ?: [[NSMutableDictionary alloc] init];
    ...
    - (UIColor *)getTintColor {
        ...
        return [GcColorPickerUtils colorWithHex:[self.preferences objectForKey:@"customTintColor"]];
    }
    ...
    - (void)convertNotesFromPreviousVersions {
        ...
        if([defaultManager fileExistsAtPath:jbroot(@"/User/Library/Preferences/LibellumNotes.txt")]) {
            NSString *contents = [NSString stringWithContentsOfFile:jbroot(@"/User/Library/Preferences/LibellumNotes.txt") encoding:NSUTF8StringEncoding error:nil];
            ...
            [defaultManager removeItemAtPath:jbroot(@"/User/Library/Preferences/LibellumNotes.txt") error:nil];
            [defaultManager removeItemAtPath:jbroot(@"/User/Library/Preferences/LibellumNotes.bk") error:nil];

            numberOfConvertedNotes++;
        }

        // Last form of notes were stored in a rtf file
        if([defaultManager fileExistsAtPath:jbroot(@"/User/Library/Preferences/LibellumNotes.rtf")]) {
            NSMutableAttributedString *content = [[NSMutableAttributedString alloc] initWithData:[NSData dataWithContentsOfFile:jbroot(@"/User/Library/Preferences/LibellumNotes.rtf")] options:@{NSDocumentTypeDocumentAttribute: NSRTFTextDocumentType} documentAttributes:nil error:nil];
